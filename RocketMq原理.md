...
![你可能会采用这种方式保证消息顺序](https://i.imgur.com/Y8oP4q9.png)
...
![保证消息顺序的正确姿势](https://i.imgur.com/39hiRqW.png)
...
* 这样的设计虽然简单易行，但也会存在一些很严重的问题，比如：并行度就会成为消息系统的瓶颈（吞吐量不够）
...
![单机环境下转账事务示意图](https://i.imgur.com/BnxKndk.png)


当用户增长到一定程度，Bob和Smith的账户及余额信息已经不在同一台服务器上了，那么上面的流程就变成了这样：
![集群环境下转账事务示意](https://i.imgur.com/mMAh8cj.png)
...
![小事务+异步消息](https://i.imgur.com/7ydSWoI.png)
...
![事务消息-先发送消息](https://i.imgur.com/3P3GcYU.png)


存在的问题是：如果消息发送成功，但是扣款失败，消费端就会消费此消息，进而向Smith账户加钱。
先发消息不行，那就先扣款吧，大致的示意图如下：
![事务消息-先扣款](https://i.imgur.com/RwTg0oO.png)
...
![RocketMQ实现发送事务消息](https://i.imgur.com/gNSUa0h.png)
...
![消费事务消息](https://i.imgur.com/D9L6xit.png)
...
![producer发送消息负载均衡](https://i.imgur.com/fZNzXZ8.png)
...
![Consume Queue文件组织示意图](https://i.imgur.com/nGViKox.png)
...
![consumequeue文件存储单元格式](https://i.imgur.com/UVdWyJU.png)
...
![Commit Log存储单元结构图](https://i.imgur.com/5mQ8efr.png)
...
![消息索引](https://i.imgur.com/14PrbDz.png)
...
首先看下消费端的负载均衡：
![消费端负载均衡](https://i.imgur.com/ZvPfaYp.png)
...
![Push模式示意图](https://i.imgur.com/mDmOiWZ.png)
